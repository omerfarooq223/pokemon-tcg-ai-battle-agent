"""One-change screen: preserve colored attack costs when paying retreat."""
...
_SPEC = importlib.util.spec_from_file_location("v9_retreat_energy_base", _BASE_PATH)
...
def score_option(obs, option):
    if (
        option.get("type") == 6
        and _base.select_state(obs).get("context") == 30
    ):
