...
def should_escape_froslass_checkup(obs):
    """Whether attacking in place guarantees our Active is KO'd in Checkup.

    Froslass's Freezing Shroud places one counter on every non-Froslass
    Pokémon with an Ability during Pokémon Checkup.  If the Active is already
    inside that deterministic range, a normal attack can cause an avoidable
    post-attack knockout.  Retreat only when a ready Benched attacker survives
    the same check; this deliberately avoids reopening the broad retreat
    heuristic that failed earlier gates.
    """
    state = current_state(obs)
    yi = your_index(state)
    ...
    if not isinstance(active, dict) or card_id(active) not in ABILITY_POKEMON_IDS:
        return False
    checkup_damage = 10 * froslass_count
    if as_int(active.get("hp"), 0) > checkup_damage:
        return False

    # Do not delay an attack that takes our final Prize cards before Checkup.
    target = active_card(state, opponent)
    if isinstance(target, dict):
        target_hp = as_int(target.get("hp"), 0)
        attack_damage = readiness(active)["damage"]
        prize_value = 2 if card_id(target) in EX_POKEMON else 1
        ps = players(state)
        own_remaining = 6
        if yi < len(ps):
            own_state = ps[yi]
            if own_state.get("prizeCount") is not None:
                own_remaining = as_int(own_state.get("prizeCount"), 6)
            elif isinstance(own_state.get("prize"), list) and own_state.get("prize"):
                own_remaining = len(own_state["prize"])
        if (
            target_hp > 0
    ...
    for card in card_list(state, yi, 5, {}):
        if not isinstance(card, dict) or not readiness(card)["ready"]:
            continue
        candidate_checkup = (
            checkup_damage if card_id(card) in ABILITY_POKEMON_IDS else 0
        )
        if as_int(card.get("hp"), 0) > candidate_checkup:
            return True
    return False
...
def bounded_setup_choice(obs, ranked):
    ...
    if should_escape_froslass_checkup(obs):
        choice = best_index(
            lambda index: options[index].get("type") == 12
        )
