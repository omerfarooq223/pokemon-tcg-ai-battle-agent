"""Cumulative V9 plus the two low-frequency mechanical safeguards."""
...
_FROSLASS_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "v9_cumulative_froslass",
    "main.py",
)
_SPEC = importlib.util.spec_from_file_location("v9_final_safeguards_base", _FROSLASS_PATH)
if _SPEC is None or _SPEC.loader is None:
    raise RuntimeError("Unable to load the cumulative Froslass candidate")
_froslass = importlib.util.module_from_spec(_SPEC)
_SPEC.loader.exec_module(_froslass)
_base = _froslass._base
...
def choose_action(obs):
    return _froslass.choose_action(obs)
