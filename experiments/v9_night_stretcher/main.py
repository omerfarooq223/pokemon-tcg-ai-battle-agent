...
EXPECTED_DECK = [
    ...
    1086, 1097, 1152, 1152, 1152, 1152, 1198, 1198, 1198, 1227,
...
def remaining_prize_count(state, player):
    """Read remaining prizes from either supported observation shape.

    Live replay observations expose ``prize`` as a list and do not include the
    older ``prizeCount`` key.  Treat an empty setup-time list as the normal six
    prizes; once play begins, the list length is the authoritative count.
    """
    ps = players(state)
    if player < 0 or player >= len(ps):
        return 6
    player_state = ps[player]
    if player_state.get("prizeCount") is not None:
        return as_int(player_state.get("prizeCount"), 6)
    prize = player_state.get("prize")
    if isinstance(prize, list) and prize:
        return len(prize)
    return 6


def opponent_prize_count(obs):
    """Keep legacy opponent-pressure behavior; exact count is not actionable."""
    return 6


def our_prize_count(obs):
    """Return our remaining Prize cards."""
    state = current_state(obs)
    yi = your_index(state)
    return remaining_prize_count(state, yi)
...
def score_target_selection(obs, option):
    ...
    if context == 7 and effect_id == 1097 and owner == yi and area == 3:
        # Night Stretcher is a resilience card, so recover the piece that can
        # create the next complete attacker line instead of blindly taking the
        # highest generic role (or an Energy due to the discard-area bonus).
        board = board_cards_only(state, yi)
        board_ids = [card_id(candidate) for candidate in board]
        hand = hand_ids(state, yi)
        if cid == 345 and 344 in board_ids and 345 not in hand:
            return score + 1800.0
        if cid == 344 and (
            len(board) <= 1 or not ({344, 345} & set(board_ids))
        ):
            return score + 1700.0
        if cid == 117 and 117 not in board_ids:
            active_ability, any_ability = opponent_ability_pressure(obs)
            if active_ability or any_ability:
                return score + 1200.0
        if cid in POKEMON_ROLE:
            return score + 500.0 + POKEMON_ROLE.get(cid, 0.0)
        if cid in ENERGY_CARDS:
            fit = energy_board_fit_score(obs, cid)
            return score + 250.0 + max(0.0, fit) * 0.5
        return score
    if (
        context == 7
        and effect_id == 1198
...
def bounded_setup_choice(obs, ranked):
    ...
    if board_size == 1:
        ...
        if choice is None:
            discard_ids = {
                card_id(candidate)
                for candidate in card_list(state, yi, 3, {})
            }
            if discard_ids & ({344, 345, 117} | ENERGY_CARDS):
                choice = best_index(
                    lambda index: options[index].get("type") == 7
                    and play_id(index) == 1097
                )
