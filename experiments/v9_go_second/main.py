...
def score_option(obs, option):
    ...
    if select_state(obs).get("context") == 41 and option_type in (1, 2):
        # Dwebble can attach and use Ascension immediately when we go second,
        # while the first player cannot attack on their opening turn.  Player
        # 0 makes this choice; option type 2 gives the first turn to player 1.
        return 1000.0 if option_type == 2 else 0.0
    if option_type == 0 and select_state(obs).get("context") == 38:
        # DRAW_COUNT at setup is the optional compensation for an opponent's
        # mulligans.  The legacy tie-break always selected option 0 and gave
        # up free opening cards; prefer the largest offered count.
