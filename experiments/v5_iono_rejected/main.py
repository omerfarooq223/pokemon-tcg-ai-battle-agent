...
ATTACKS = {
    265: [{"cost": ["C", "C"], "damage": 20, "name": "Voltaic Chain"}],
    268: [{"cost": ["L", "C"], "damage": 30, "name": "Tiny Charge"}],
    269: [{"cost": ["L", "L", "L", "C"], "damage": 230, "name": "Thunderous Bolt"}],
    270: [{"cost": ["L"], "damage": 10, "name": "Quick Attack"}],
    271: [{"cost": ["L", "C", "C"], "damage": 70, "name": "Mach Bolt"}],
...
DRAW_SEARCH_CARDS = {
    1088, 1097, 1098, 1110, 1118, 1121, 1152, 1198, 1205, 1233, 1250,
}
DISRUPTION_CARDS = {1182, 1197}
ENERGY_CARDS = set(ENERGY_CARD_TYPES)
ABILITY_POKEMON = {96, 269, 271, 756, 1071, 140, 184, 272}
BASIC_SETUP_POKEMON = {117, 265, 268, 270, 344, 397, 796, 1073}
...
def score_play_from_hand(obs, option):
    ...
    if cid in DRAW_SEARCH_CARDS:
        score += 180.0
    own_ids = [card_id(card) for card in board_cards_only(state, yi)]
    if cid == 1086 and (
        own_ids.count(268) + own_ids.count(270) < 3
    ):
        score += 900.0
    if cid == 1152 and 268 not in own_ids:
        score += 520.0
    if cid == 1233 and (268 not in own_ids or 269 not in own_ids):
        score += 780.0
    ...
    if cid in POKEMON_ROLE:
        ...
        if len(bench) >= 8:
            ...
            if cid == 268 and 269 not in own_ids:
                score += 480.0
            elif cid == 265 and 265 not in own_ids:
                score += 180.0
...
def energy_fit_bonus(target, energy_id):
    ...
    if target_id == 265:
        # Every Lightning Energy on any Iono Pokémon increases Voltaic Chain.
        return 220.0 if len(attached) < 2 else 95.0
    if target_id == 269:
        return 185.0 if energy_type == "L" and len(attached) < 4 else 75.0
    if target_id == 271:
        return 150.0 if energy_type == "L" and len(attached) < 3 else 70.0
    if target_id in (268, 270):
        return 130.0 if energy_type == "L" and not attached else 55.0
    if target_id == 96:
...
def card_pick_score(obs, cid, area, context):
    ...
    own_ids = [card_id(card) for card in board_cards_only(state, yi)]
    if cid == 268:
        # Establish multiple Tadbulb so losing the opening Basic does not turn
        # every Bellibolt in hand into a dead card.
        if 269 not in own_ids:
            score += 320.0
        if own_ids.count(268) + own_ids.count(269) < 2:
            score += 220.0
    elif cid == 269:
        score += 360.0 if 268 in own_ids else 80.0
    elif cid == 265 and 265 not in own_ids:
        score += 150.0
    if cid in DRAW_SEARCH_CARDS:
        score += 130.0
...
def bounded_setup_choice(obs, ranked):
    ...
    if choice is None:
        # Bellibolt's Electric Streamer is the engine of this deck. It may be
        # used repeatedly, and each attachment also increases Voltaic Chain.
        choice = best_index(
            lambda index: options[index].get("type") == 10
            and card_id(option_card(obs, options[index])) in {269, 271, 1254}
        )
