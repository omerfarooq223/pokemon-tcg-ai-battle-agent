...
EXPECTED_DECK = [
    ...
    1086, 1087, 1152, 1152, 1152, 1152, 1198, 1198, 1198, 1227,
...
def score_play_from_hand(obs, option):
    ...
    if cid == 1087:
        our_hand = len(hand)
        if opponent_hand <= 5 and our_hand <= 5:
            return -850.0
        # Hand Trimmer is weaker than Xerosic but is an Item, so it can provide
        # a second, generally useful disruption route without consuming the
        # Supporter action.  Penalize throwing away our own excess cards.
        score += max(0, opponent_hand - 5) * 160.0
        score -= max(0, our_hand - 5) * 45.0
        if opponent_hand >= 8:
            score += 320.0
    if cid == 1198:
        score += 120.0
...
def bounded_setup_choice(obs, ranked):
    ...
    if choice is None and hand_count(state, 1 - yi) >= 5:
        ...
        if choice is None and hand_count(state, 1 - yi) >= 6:
            choice = best_index(
                lambda index: options[index].get("type") == 7
                and play_id(index) == 1087
            )
