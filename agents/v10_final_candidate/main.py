"""V10 Final Mega Lopunny / Dudunsparce state planner.
...
# Mega Evolution Pokemon ex give up 3 Prize cards rather than the ordinary
# Pokemon ex 2.  Prize-aware targeting must distinguish the two rule boxes.
MEGA_EX_POKEMON = {
...
def current_attack_damage_against(state, target):
    """Return the best damage the current Active can deal to this target."""
    yi = your_index(state)
...
def discard_keep_value(obs, card, option):
    ...
    if cid == LOPUNNY:
        ...
        # Wally is the deck's repeatable three-Prize-attacker recovery engine,
        # not a disposable situational Supporter. Preserve at least one copy
        # whenever a Lopunny line exists, even before it takes damage.
        lopunny_lines = counts.get(BUNEARY, 0) + counts.get(LOPUNNY, 0)
        damaged_lopunny = any(
            damage_on(c) >= 80
            for _, _, c in board(state, yi)
            if card_id(c) == LOPUNNY
        )
...
def energy_available_after_wally(state, target):
    yi = your_index(state)
    return attached_count(target) > 0 or any(
        cid in ENERGY_IDS for cid in hand_ids(state, yi)
    )


def wally_target_value(state, card, area, retreat_available=False):
    """Score a Wally target only when the resulting turn remains coherent."""
    if card_id(card) != LOPUNNY:
        return -12000.0
    ...
    yi = your_index(state)
    if area == 5:
        # Healing a Benched three-Prize attacker never strands the Active.
    ...
    protection_break_needed = (
        card_id(active(state, 1 - yi)) in DAMAGE_PROTECTION_POKEMON
    )

    # Safe active-heal routes:
    # 1. reattach to this Active and attack;
    # 2. free-retreat into an already ready Lopunny;
    # 3. use the returned Energy to power a Benched Lopunny, then free-retreat
    #    into it for the 230-damage Gale Thrust pivot.
    if protection_break_needed:
        # Wally returns every attached Energy.  Against a protection Ability,
        # one reattachment cannot restore Spiky Hopper in the same turn, and a
        # one-Energy Gale Thrust pivot still deals no damage.  Preserve the
        # current legal Spiky attack unless a different two-Energy Lopunny can
        # take over immediately.
        coherent = (
            retreat_available
            and can_retreat
            and free_retreat
            and spiky_ready_bench
        )
    else:
        coherent = (
...
def supporter_value(obs, cid, from_search=False):
    ...
    boss_targets = [
        c for c in opponent_bench
        if 0 < hp(c) <= current_attack_damage_against(state, c)
    ]
    ...
    if cid == BOSS:
        ...
        retreat_available = any(
            option.get("type") == 12
            for option in selection(obs).get("option") or []
        )
        values = [
            wally_target_value(state, card, area, retreat_available)
            for area, _, card in board(state, yi)
        ]
...
def attach_score(obs, option):
    ...
    # Complete the Wally engine instead of immediately repowering the healed
    # Active by default.  When that Active has Air Balloon, powering a Benched
    # Lopunny creates the free pivot that both preserves the healed attacker
    # and activates 230-damage Gale Thrust.  If Wally healed a Benched
    # attacker, rebuild it while the current Active remains ready.
    target_serial = target.get("serial") if isinstance(target, dict) else None
    our_active = active(state, yi)
    active_serial = (
        our_active.get("serial") if isinstance(our_active, dict) else None
    )
    if WALLY_HEALED_SERIAL is not None:
        if WALLY_HEALED_WAS_ACTIVE and active_serial == WALLY_HEALED_SERIAL:
            ...
            if (
                ...
                and any(
                    candidate.get("type") == 12
                    for candidate in selection(obs).get("option") or []
                )
                and area == 5
            ...
            not WALLY_HEALED_WAS_ACTIVE
            and target_serial == WALLY_HEALED_SERIAL
            and area == 5
        ):
            score += 4200.0
...
def target_selection_score(obs, option):
    ...
    if owner == yi and area in (4, 5):
        ...
        if context == 17 and effect_id == WALLY:
            return wally_target_value(
                state, card, area, WALLY_RETREAT_WAS_AVAILABLE
            )
...
def safe_action(obs):
    ...
    try:
        ...
        for index in action:
            ...
            if (
                option.get("type") == 7
                and card_id(option_source(obs, option)) == WALLY
            ):
                WALLY_RETREAT_WAS_AVAILABLE = any(
                    candidate.get("type") == 12 for candidate in options
                )
            if select.get("context") == 17:
                target = option_source(obs, option)
                if card_id(target) == LOPUNNY:
                    WALLY_HEALED_SERIAL = target.get("serial")
