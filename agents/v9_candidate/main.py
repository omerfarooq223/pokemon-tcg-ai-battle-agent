...
def should_escape_froslass_checkup(obs):
    """Retreat a doomed Ability Active only to an equally strong survivor."""
    state = current_state(obs)
    yi = your_index(state)
    ...
    froslass_count = sum(
        card_id(card) == 104
        for _, _, _, card in board_cards(state, opponent)
    )
    if froslass_count <= 0:
        return False
    ...
    if (
        ...
        or card_id(active) not in ABILITY_POKEMON_IDS
    ):
        return False
    checkup_damage = 10 * froslass_count
    if as_int(active.get("hp"), 0) > checkup_damage:
        return False

    target = active_card(state, opponent)
    if isinstance(target, dict):
        target_hp = as_int(target.get("hp"), 0)
        active_damage = readiness(active)["damage"]
        prize_value = 2 if card_id(target) in EX_POKEMON else 1
        ps = players(state)
        own_remaining = 6
        if yi < len(ps) and isinstance(ps[yi].get("prize"), list):
            own_remaining = len(ps[yi]["prize"])
        if (
            target_hp > 0
    ...
    active_damage = readiness(active)["damage"]
    for card in card_list(state, yi, 5, {}):
        if not isinstance(card, dict):
            continue
        candidate = readiness(card)
        candidate_checkup = (
            checkup_damage if card_id(card) in ABILITY_POKEMON_IDS else 0
        )
        if (
            candidate["ready"]
            and candidate["damage"] >= active_damage
            and as_int(card.get("hp"), 0) > candidate_checkup
        ):
            return True
    return False
...
def is_protective_mist_option(obs, option):
    """Whether this legal attachment protects the Active from a profiled effect."""
    if option.get("type") != 8:
        return False
    state = current_state(obs)
    yi = your_index(state)
    target = target_card(obs, option)
    opponent_id = card_id(active_card(state, 1 - yi))
    return (
        card_id(option_card(obs, option)) == 11
        and option.get("inPlayArea") == 4
        and isinstance(target, dict)
        and opponent_id in MIST_ACTIVE_THREATS
        and not any(
            card_id(energy) == 11 for energy in target.get("energyCards") or []
        )
    )


def is_active_dwebble_acceleration_option(obs, option):
    """Whether an attachment prepares the Crustle that Ascension will create."""
    state = current_state(obs)
    target = target_card(obs, option)
    return (
        option.get("type") == 8
        and card_id(option_card(obs, option)) in ENERGY_CARDS
        and option.get("inPlayArea") == 4
        and card_id(target) == 344
        and len(attached_types(target)) < 3
...
def score_attach_or_evolve(obs, option):
    ...
    if cid in ENERGY_CARDS:
        if is_protective_mist_option(obs, option):
            # Mist is attached for effect protection, not additional attack
            # cost, so this must precede the generic fully-paid cutoff.
            return 3600.0
        if is_active_dwebble_acceleration_option(obs, option):
            # Ascension keeps attached Energy. Evaluate this attachment against
            # Crustle's eventual {G}{C}{C} attack instead of treating
            # one-Energy Dwebble as already fully paid.
            evolved = dict(target)
            evolved["id"] = 345
            before_evolution = readiness(evolved)
            after_evolution = readiness(evolved, extra_energy=cid)
            score = (
                1300.0
                + (after_evolution["score"] - before_evolution["score"]) * 3.0
            )
            if after_evolution["ready"] and not before_evolution["ready"]:
                score += 900.0
            elif after_evolution["missing"] == 1:
                score += 450.0
            if cid == 18:
                score += 260.0
            elif cid == 1:
...
def discard_preservation_score(obs, option):
    ...
    yi = your_index(state)
    if (
        select.get("context") not in {8, 29}
...
def card_pick_score(obs, cid, area, context):
    ...
    if effect_id == 1152 and area == 1 and cid == 344:
        board = board_cards_only(state, yi)
        visible_ids = {card_id(card) for card in board} | set(hand)
        offered_ids = {
            card_id(card)
            for card in (select_state(obs).get("deck") or [])
            if isinstance(card, dict)
        }
        if (
            len(board) == 1
            and not ({344, 345} & visible_ids)
            and {344, 345}.issubset(offered_ids)
        ):
            # A Stage 1 is unusable on a one-Pokémon board without Dwebble.
            # Establish the missing evolution line first.
            score += 900.0
    if (
        effect_id == 1152
        and area == 1
...
def score_option(obs, option):
    ...
    if option_type == 0 and select_state(obs).get("context") == 38:
        # Draw the maximum free compensation offered after an opponent mulligan.
        return as_int(option.get("number"), 0) * 100.0
    if option_type == 13:
...
def bounded_setup_choice(obs, ranked):
    ...
    if should_escape_froslass_checkup(obs):
        choice = best_index(lambda index: options[index].get("type") == 12)
    if choice is None:
        choice = best_index(
            lambda index: is_protective_mist_option(obs, options[index])
        )
    if choice is None:
        choice = best_index(
            lambda index: is_active_dwebble_acceleration_option(
                obs, options[index]
            )
        )
    if choice is None and board_size == 1:
        choice = best_index(
