...
def attached_types(card):
    return [
        ENERGY_TYPES[energy_id]
        for energy_id in (card or {}).get("energies") or []
        if energy_id in ENERGY_TYPES
    ]
...
def readiness_score(card, extra_energy_id=None):
    ...
    if extra_energy_id in ENERGY_TYPES:
        energies.append(ENERGY_TYPES[extra_energy_id])
...
def energy_attachment_bonus(obs, option):
    ...
    if source_id not in ENERGY_TYPES:
        return 0.0
    target = full_card_at(
        state,
        option.get("playerIndex", your_index),
