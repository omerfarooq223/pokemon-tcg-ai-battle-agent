"""V14 mechanics-aware Crustle / Cornerstone resilience planner.

Improvements over V13:
1. Anti-Bench-Wipe Safeguard: Strict invariant to maintain at least 1-2 Benched
   Pokémon. Prevents instant KO losses when bench_count == 0.
2. Alakazam & Hand Control Response: Threat detection for Alakazam/Dudunsparce
   engines, prioritizing Xerosic's Machinations when opp hand >= 5.
3. Enhanced Attacker & Energy Placement: Optimized Crispin/Waitress energy
   targeting and attack readiness logic.
4. Kaggle Loader & Runtime Invariants: Bug-free execution without __file__.
"""
...
# Cost symbols use C for colorless. The profiles cover our deck's attackers.
ATTACKS = {
...
# Expanded damage counter and hand-scale pressure targets:
# Alakazam (15, 16, 17, 72, 163), Dudunsparce (1071), Munkidori (743), Dragapult ex (648)
DAMAGE_COUNTER_PRESSURE = {15, 16, 17, 72, 112, 121, 163, 648, 743, 1071}

# Attackers whose moves bypass Active effects (e.g. Mega Lopunny ex 849 Spiky Hopper):
...
def required_line_count(obs):
    ...
    visible_opponent = board_cards(state, opp)
    if any(
        card_id(card) in DAMAGE_COUNTER_PRESSURE
        for _, _, _, card in visible_opponent
    ):
        return 3
    return 2
...
def opponent_bypass_pressure(obs):
    ...
    any_bypass = any(
        card_id(card) in BYPASS_EFFECT_ATTACKERS for _, _, _, card in cards
    )
...
def score_play_from_hand(obs, option):
    ...
    if cur_bench == 0:
        if cid in BASIC_SETUP_POKEMON:
            return 25000.0  # Ultra-priority to place basic on bench immediately
        if cid == 1086:
            capacity = poffin_progress_capacity(obs)
            if capacity > 0:
                return 22000.0  # Priority to search basic pokemon via Poffin
        if cid == 1152:
            return 18000.0  # Priority to Poke Pad for basic/supporter
    ...
    if cid == 1197:  # Xerosic's Machinations
        if opponent_hand <= 3:
            return -850.0
        score += (opponent_hand - 3) * 220.0
        if opponent_hand >= 5:
            score += 1200.0  # Force opponent hand drop from 5+ down to 3

    # Active healing priority to preserve active attacker
    ...
    if cid in (1147, 1212) and active_missing_hp >= 40:
        score += 850.0  # Heal active when missing 40+ HP to prevent 2-hit KOs

    active_bypass, any_bypass = opponent_bypass_pressure(obs)
    if cid == 1159:  # Hero's Cape (+100 HP)
        if active_bypass or any_bypass:
            score += 1500.0  # Priority to boost HP against effect bypass attackers
...
def score_energy_attachment(obs, option):
    ...
    # Grow Grass Energy (18) priority on Dwebble/Crustle
    if cid == 18:
    ...
    # Mist Energy (11) priority facing effect threats
    if cid == 11:
    ...
    # Spiky Energy (14) priority on active
    if cid == 14:
    ...
    # Cornerstone (117) vs Crustle (345) matchup adaptability
    if target_cid == 117:
        if active_bypass or any_bypass:
            score += 550.0  # Cornerstone's 210/310 HP outlives effect-bypassing attackers like Lopunny ex
        ...
        if active_bypass or any_bypass:
            score -= 150.0  # Crustle's immunity is bypassed by Spiky Hopper / Shred, prefer Cornerstone
        elif active_ex:
            score += 220.0
...
def score_option(obs, option):
    ...
    if option_type == 13:
        ...
        if opp_id in DAMAGE_COUNTER_PRESSURE:
            score += 500.0  # High priority to KO Alakazam / Dudunsparce / Munkidori

        # Ascension bonus for active Dwebble (evolves Dwebble into Crustle from deck)
        our_active = active_card(state, your_index(state))
        if isinstance(our_active, dict) and card_id(our_active) == 344 and damage == 0:
            score += 350.0  # Priority to evolve active Dwebble via Ascension
...
def bounded_setup_choice(obs, ranked):
    ...
    cur_bench = bench_count(state, yi)
    # INVARIANT: If bench_count == 0, allow setup action to prevent Bench Wipe KO!
    ...
    # Anti-Bench Wipe override when bench_count == 0
    if cur_bench == 0:
        choice = best_index(
...
def choose_action(obs):
    ...
    if card_id(select.get("effect")) == 1086:
        ...
        # If cur_bench == 0, take maximum allowed Basic Pokemon
