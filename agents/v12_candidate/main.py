"""V12 recency-weighted Crustle / Cornerstone resilience planner.
...
# Powerful Hand and Shadow Bullet place damage counters rather than relying on
# ordinary attack damage.  Crustle/Cornerstone protection therefore does not
# make a two-Pokemon board safe: one counter-placement knockout can leave the
# next Active exposed.  This is a mechanic-based pressure class, not an
# opponent or replay identity rule.
DAMAGE_COUNTER_PRESSURE = {648, 743}
...
EXPECTED_DECK = [
    ...
    1227, 1227, 1227, 1197, 1197, 1197, 1197, 1235, 1235, 1235,
...
def required_line_count(obs):
    ...
    opp = 1 - your_index(state)
    active = active_card(state, opp)
    if card_id(active) in DAMAGE_COUNTER_PRESSURE:
        return 3
    return 2
