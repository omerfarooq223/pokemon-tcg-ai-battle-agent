"""V13 mechanics-aware Crustle / Cornerstone resilience planner.
...
# These visible Pokémon can damage or move damage onto a second target.  The
# protection abilities in our deck do not make a two-Pokémon board safe from
# that pressure, so establish a third viable line when any is already in play.
# This is a mechanics class, not an opponent or replay identity rule.
DAMAGE_COUNTER_PRESSURE = {112, 121, 648, 743}
...
def required_line_count(obs):
    """Return the minimum viable attacker lines to establish before setup.

    Ordinary boards retain V11's two-line opening.  Against visible counter or
    bench-spread attackers, a third line is worth the extra search because the
    protection abilities in this deck do not prevent damage counters.
    """
    state = current_state(obs)
    opp = 1 - your_index(state)
...
def emergency_action(obs):
    """Return a conservative legal action without calling policy scorers.

    Competition safety still requires a last-resort action if an unforeseen
    observation shape reaches the planner. Prefer a legal attack, otherwise
    satisfy only the mandatory selection count.
    """
