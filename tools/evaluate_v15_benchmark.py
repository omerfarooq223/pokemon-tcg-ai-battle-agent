#!/usr/bin/env python3
"""Memory-safe benchmark evaluation runner for V15 Candidate Agent on 8GB RAM Mac.

Executes head-to-head match batches in isolated subprocesses to keep RAM footprint
under 500 MB and prevent memory accumulation.
"""
...
def run_batch(v15_dir: Path, opp_dir: Path, matches: int, swap_seats: bool) -> tuple[int, int, int]:
    cmd = [
        sys.executable,
        str(ROOT / "tools" / "run_local_matches.py"),
        "--agent-dir", str(v15_dir),
        "--agent-deck", str(v15_dir / "deck.csv"),
        "--opponent", "agent",
...
def evaluate_opponent(v15_dir: Path, opp_name: str, opp_dir: Path, total_matches: int, batch_size: int):
    print(f"\n==========================================")
    print(f"  Evaluating V15 vs {opp_name.upper()} ({total_matches} matches, batch_size={batch_size})")
    print(f"==========================================")
    ...
    while remaining > 0:
        ...
        batch_count += 1
        w, l, d = run_batch(v15_dir, opp_dir, current_batch, swap_seats=True)
        total_wins += w
        total_losses += l
        total_draws += d
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate V15 against existing agents in memory-safe batches.")
    parser.add_argument("--v15-dir", type=Path, default=ROOT / "agents" / "v15_candidate")
    parser.add_argument("--opponents", nargs="+", default=["v14", "v13", "v12", "v11", "v5", "v4", "v1"])
    ...
    for opp_name in args.opponents:
        if opp_name in OPPONENTS:
            ...
            w, l, d = evaluate_opponent(args.v15_dir, opp_name, opp_dir, args.matches, args.batch_size)
            results[opp_name] = (w, l, d)
        else:
            print(f"Unknown opponent {opp_name}, skipping.")

    print("\n==========================================")
    print("           V15 BENCHMARK SUMMARY          ")
    print("==========================================")
    for opp_name, (w, l, d) in results.items():
        total = w + l + d
        rate = (w / total * 100.0) if total > 0 else 0.0
        print(f"  V15 vs {opp_name.upper():5s}: {w:4d} W | {l:4d} L | {d:4d} D ({rate:6.2f}%)")
    print("==========================================")
