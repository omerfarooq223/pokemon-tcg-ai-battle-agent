#!/usr/bin/env python3
"""Memory-safe benchmark evaluation runner for V16 Candidate Agent on 8GB RAM Mac.
...
def run_batch(v16_dir: Path, opp_dir: Path, matches: int, swap_seats: bool) -> tuple[int, int, int]:
    cmd = [
        sys.executable,
        str(ROOT / "tools" / "run_local_matches.py"),
        "--agent-dir", str(v16_dir),
        "--agent-deck", str(v16_dir / "deck.csv"),
        "--opponent", "agent",
...
def evaluate_opponent(v16_dir: Path, opp_name: str, opp_dir: Path, total_matches: int, batch_size: int):
    print(f"\n==========================================")
    print(f"  Evaluating V16 vs {opp_name.upper()} ({total_matches} matches, batch_size={batch_size})")
    print(f"==========================================")
    ...
    while remaining > 0:
        ...
        batch_count += 1
        w, l, d = run_batch(v16_dir, opp_dir, current_batch, swap_seats=True)
        total_wins += w
        total_losses += l
        total_draws += d
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate V16 against existing agents in memory-safe batches.")
    parser.add_argument("--v16-dir", type=Path, default=ROOT / "agents" / "v16_candidate")
    parser.add_argument("--opponents", nargs="+", default=["v15", "v14", "v13", "v12", "v11", "v5", "v4", "v1"])
    ...
    for opp_name in args.opponents:
        if opp_name in OPPONENTS:
            ...
            w, l, d = evaluate_opponent(args.v16_dir, opp_name, opp_dir, args.matches, args.batch_size)
    ...
    for opp_name, (w, l, d) in results.items():
        ...
        print(f"  V16 vs {opp_name.upper():5s}: {w:4d} W | {l:4d} L | {d:4d} D ({rate:6.2f}%)")
    print("==========================================")
